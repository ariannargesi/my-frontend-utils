...
export class ThemeHandler {
  ...
  private static applySystemDefault = () => {
    ...
    if (isOS_dark) this.applyDark()
    else this.applyLight()
  }

  static changeTheme = (value: Themes) => {
    if (value === "light") this.applyLight()
    else if (value === "dark") this.applyDark()
  ...
  static initiateTheme = () => {
    ...
    if (
      ...
      return
    if (defaultTheme === "light") this.changeTheme("light")
    else if (defaultTheme === "dark") this.changeTheme("dark")
    else this.changeTheme("system")
  }

  static getTheme = (): Themes =>
    (localStorage.getItem("theme-mode") || "system") as Themes
}
